...
BASE_DIR = "/maps/projects/rasmussen/data/taxvamb_benchmarks/taxvamb_benchmarks/resubmission/data/erda/reads"
# name = "erda"
dataset = ""
OUTDIR = OUTDIR / "erda"
...
SAMPLES = sorted(list(set([os.path.basename(f).split('_')[0] for f in os.listdir(BASE_DIR) if f.endswith('.fq.gz')])))
print(SAMPLES)
...
def get_fastq_reads(wildcards):
    ...
    fw_read = os.path.join(BASE_DIR, f"{wildcards.sample}_R1.fq.gz")
    rv_read = os.path.join(BASE_DIR, f"{wildcards.sample}_R2.fq.gz")
    return [fw_read, rv_read] # Return a list of paths
def get_contig(wildcards):
    print("wildcards",wildcards)
    contig = os.path.join(Path(BASE_DIR).parent / "assemblies" , f"{wildcards.sample}.fasta.gz")
    return [contig] # Return a list of paths
...
rule cat_contigs_2:
    input: 
        contig = expand(os.path.join(Path(BASE_DIR).parent / "assemblies" ,"{sample}.fasta.gz"), sample=SAMPLES)
    output: OUTDIR / "spades_output"/ dataset / "contigs.flt.fna.gz"
    threads: threads_fn(rulename)
    params: script =  SRC_DIR / "concatenate.py"
    resources: walltime = walltime_fn(rulename), mem_gb = mem_gb_fn(rulename), gpu=gpu_fn(rulename)
    conda: THIS_FILE_DIR / "envs/vamb.yaml"
...
rule index:
    input:
        contigs = OUTDIR / "spades_output" / dataset / "contigs.flt.fna.gz"    
    output:
        mmi = os.path.join(OUTDIR, "spades_output", dataset, "contigs.flt.mmi")
    threads: threads_fn(rulename)
    resources: walltime = walltime_fn(rulename), mem_gb = mem_gb_fn(rulename), gpu=gpu_fn(rulename)
    conda: 
        THIS_FILE_DIR / "envs/minimap2.yaml"
...
rule dict:
    input:
        contigs = OUTDIR / "spades_output" / dataset /  "contigs.flt.fna.gz"    
    output:
        dict = os.path.join(OUTDIR,"spades_output", dataset, "contigs.flt.dict")  
    threads: threads_fn(rulename)
    resources: walltime = walltime_fn(rulename), mem_gb = mem_gb_fn(rulename), gpu=gpu_fn(rulename)
    conda:
        THIS_FILE_DIR / "envs/minimap2.yaml"
